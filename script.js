...
async function SelectionSort(array) {
    ...
    for (let i = 0; i < array.length - 1; i++) {
      let minIndex = i;
      bars[i].style.backgroundColor = "red"; // Mark the starting point
      for (let j = i + 1; j < array.length; j++) {
        bars[j].style.backgroundColor = "yellow"; // Mark the current comparison bar
        await sleep(speedFactor);
  
        if (array[j] < array[minIndex]) {
          if (minIndex !== i) {
            bars[minIndex].style.backgroundColor = "blue"; // Revert the previous minimum bar color
          }
          minIndex = j;
          bars[minIndex].style.backgroundColor = "red"; // Mark the new minimum bar
        } else {
          bars[j].style.backgroundColor = "blue"; // Revert the color of the bar being compared
        }
      }
      ...
      bars[i].style.backgroundColor = "green"; // Mark the bar as sorted
    }
  
    bars[array.length - 1].style.backgroundColor = "green"; // Mark the last bar as sorted
    return array;
  }
  ...
  async function swap(array, i, j, bars) {
    ...
    // Update the bar heights and colors to reflect the swap
  ...
  async function bubbleSort(array) {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      bars[array.length - i - 1].style.backgroundColor = "green"; // Mark sorted bar
    }
...
async function quickSort(items, left, right) {
    let bars = document.getElementsByClassName("bar");
    if (items.length > 1) {
      let index = await partition(items, left, right, bars);
      if (left < index - 1) {
...
async function HeapSort(array) {
    ...
    for (let i = array.length - 1; i > 0; i--) {
      ...
      bars[i].style.backgroundColor = "green"; // Mark sorted bar
    }
    bars[0].style.backgroundColor = "green"; // Mark the first bar as sorted
    return array;
  }
